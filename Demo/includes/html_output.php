 ...
 */
  ...
  function href_link($page = null, $parameters = null, $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true, $use_full_address = false) {
 ...
 */
  ...
  function link_object($link, $object, $parameters = null) {
 ...
 */
  ...
  function _image($image, $title = null, $width = 0, $height = 0, $parameters = null) {
    ...
    $image = '<img src="' . output_string($image) . '" alt="' . output_string($title) . '"';

    if (!empty($title)) {
      $image .= ' title="' . output_string($title) . '"';
    }
 ...
 */
  ...
  function draw_image_submit_button($image, $title = null, $parameters = null) {
    ...
    if(file_exists($image_file))
      $image_submit = '<input type="image" src="' . output_string($image_file) . '"';
    else
      $image_submit = '<input type="image" src="' . output_string($default_image_file) . '"';

    if (!empty($title)) {
      $image_submit .= ' alt="' . output_string($title) . '" title="' . output_string($title) . '"';
    }
 ...
 */
  ...
  function draw_image_button($image, $title = null, $parameters = null) {
    global $osC_Language;

    $default_image_file = 'includes/languages/' . $osC_Language->getCode() . '/images/buttons/' . $image;
    $image_file = 'templates/' . $_SESSION['template']['code'] . '/images/buttons/languages/' . $osC_Language->getCode() . '/' . $image;
    if(file_exists($image_file))
      return _image($image_file, $title, null, null, $parameters);
    else
      return _image($default_image_file, $title, null, null, $parameters);
  }
 ...
 */
  ...
  function draw_input_field($name, $value = null, $parameters = null, $override = true, $type = 'text') {
    if (!is_bool($override)) {
      $override = true;
    }
 ...
 */
  ...
  function draw_password_field($name, $parameters = null) {
    return draw_input_field($name, null, $parameters, false, 'password');
  }
 ...
 */
  ...
  function draw_file_field($name, $show_max_size = false, $parameters = null) {
    global $osC_Language;
    ...
    $field = draw_input_field($name, null, $parameters, false, 'file');
    ...
    if ($show_max_size === true) {
      ...
      if (!empty($upload_max_filesize)) {
        $field .= '&nbsp;' . sprintf($osC_Language->get('maximum_file_upload_size'), output_string($upload_max_filesize));
      }
    }
 ...
 */
  ...
  function draw_selection_field($name, $type, $values, $default = null, $parameters = null, $separator = '&nbsp;&nbsp;') {
    if (!is_array($values)) {
      $values = array($values);
    }
    ...
    foreach ($values as $key => $value) {
      ...
      if (strpos($parameters, 'id=') === false) {
        $field .= ' id="' . output_string($name) . (sizeof($values) > 1 ? $counter : '') . '"';
      }

      if (trim($selection_value) != '') {
        $field .= ' value="' . output_string($selection_value) . '"';
      }
      ...
      if (!empty($selection_text)) {
        $field .= '<label for="' . output_string($name) . (sizeof($values) > 1 ? $counter : '') . '" class="fieldLabel">' . $selection_text . '</label>';
      }
 ...
 */
  ...
  function draw_checkbox_field($name, $values = null, $default = null, $parameters = null, $separator = '&nbsp;&nbsp;') {
    return draw_selection_field($name, 'checkbox', $values, $default, $parameters, $separator);
  }
 ...
 */
  ...
  function draw_radio_field($name, $values, $default = null, $parameters = null, $separator = '&nbsp;&nbsp;') {
    return draw_selection_field($name, 'radio', $values, $default, $parameters, $separator);
  }
 ...
 */
  ...
  function draw_hidden_field($name, $value = null, $parameters = null) {
 ...
 */
  ...
  function draw_pull_down_menu($name, $values, $default = null, $parameters = null) {
    $group = false;
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
      if (isset($values[$i]['group'])) {
        if ($group != $values[$i]['group']) {
          ...
          $field .= '<optgroup label="' . output_string($values[$i]['group']) . '">';
        }
      }
 ...
 */
  ...
  function draw_label($text, $for, $access_key = null, $required = false) {
    if (!is_bool($required)) {
      $required = false;
    }
    
    return '<label' . (!empty($for) ? ' for="' . output_string($for) . '"' : '') . (!empty($access_key) ? ' accesskey="' . output_string($access_key) . '"' : '') . '>' . output_string($text) . ($required === true ? '<em>*</em>' : '') . '</label>';
  }
 ...
 */
  ...
  function draw_date_pull_down_menu($name, $value = null, $default_today = true, $show_days = true, $use_month_names = true, $year_range_start = 0, $year_range_end = 1) {
    $year = date('Y');
    ...
    if ($show_days === true) {
      ...
      $days_select_string = draw_pull_down_menu($name . '_days', $days_array, $value['date']);
    }
    ...
    $months_select_string = draw_pull_down_menu($name . '_months', $months_array, $value['month'], $params);
    ...
    $years_select_string = draw_pull_down_menu($name . '_years', $years_array, $value['year'], $params);
