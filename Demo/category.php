...
#nameU
{
  ...
  /*********************************************/
    ...
    <div id="title1">
      ...
      <td><a class='lo' href="category.php?logOut=y">Log out</a></td>
        </tr>
      </table>
        <div id="content">
...
//            echo "</div>";
        ...
        ?>
        <div id="pageContent">
            <h1><a href="category.php">Categories</a></h1>
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
              ...
              <?php
                  ...
                  if (isset($categoryID) && $categoryID != "")// && strpos($categoryID, '_'))
                  {
                        //echo "True!";
                        $categoriesQuery = $Database->query('SELECT C.Category_ID, C.Name FROM OS_Category C WHERE C.Category_ID = :Category_ID and C.Active = 1 order by C.Name');
                        $categoriesQuery->bindInt(':Category_ID', $categoryID);
                        $categoriesQuery->execute();
                  }
                  else
                  {
                        //echo "False!";
                        $categoriesQuery = $Database->query('select C.Category_ID, C.Name FROM OS_Category C WHERE C.Active = 1 order by C.Name');
                        $categoriesQuery->execute();
                  }

                  $number_of_categories = $categoriesQuery->numberOfRows();
                  echo "Number of categories: " . $number_of_categories;
                  ...
                  while ($categoriesQuery->next())
                  {
                    ...
                    echo '    <td align="center" width="' . $width . '" valign="top" style="font-size:14px;"><u><a href="category.php?categoryID=' . $categoriesQuery->valueInt('Category_ID') . '">' . $categoriesQuery->value('Name') . '</a></u>';//</td>' . "\n";
                    ...
                    If ($number_of_products > 0)
                    {
                        ...
                        while ($productsQuery->next())
                        {
                            ...
                            echo '<br><a href="product.php?productID=' . $productsQuery->valueInt('Product_ID') . '">';
                            echo $productsQuery->value('Name') . '<br><br>Details ...</a>';
                            echo '</U></td>';
